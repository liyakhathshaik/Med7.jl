...
export MediSevenModel, load_model, Entity, Doc, ENTITY_TYPES, FALLBACK_MODELS
...
Load the MediSeven spaCy model with fallback options.
Returns a MediSevenModel instance that can process medical texts.

# Arguments
- `batch_size::Int=1`: Number of texts to process in parallel

# Returns
- `MediSevenModel`: Model instance for medical NER
...
"""
    (model::MediSevenModel)(text::String)
...
function (model::MediSevenModel)(text::String)
    ...
    try
        ...
        return MediSevenModel(nothing, model.batch_size)(text)
    end
end

"""
    (model::MediSevenModel)(texts::Vector{String})
